...
const ENCRYPTED_SECRETS_PATH string = "keys.encrypted"
...
func encryptSecretsCmd(cmd *cobra.Command, args []string) (err error) {
	...
	// create target file
	target, err := os.Create(ENCRYPTED_SECRETS_PATH)
	if err != nil {
		return
	}
	defer target.Close()
...
func decryptSecretsCmd(cmd *cobra.Command, args []string) (err error) {
	...
	data, err := ioutil.ReadFile(ENCRYPTED_SECRETS_PATH)
	if err != nil {
		return
	}
	...
	tmp, err := os.Create(TMP_TAR_PATH)
	if err != nil {
		return
	}
	defer tmp.Close()
